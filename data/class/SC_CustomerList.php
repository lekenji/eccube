...
class SC_CustomerList extends SC_SelectSql_Ex
{
    ...
    public function __construct($array, $mode = '')
    {
        ...
        if (strlen($this->arrSql['search_category_id']) > 0) {
         ...
         if (!isset($this->arrSql['search_customer_type'])) $this->arrSql['search_customer_type'] = '';
        if (strlen($this->arrSql['search_customer_type']) > 0) {
            $this->setWhere('typecustomer = ?');
            $this->arrVal[] = $this->arrSql['search_customer_type'];
//            echo '<pre>';
//            var_dump($this->getSql());
//            die();
            // query dtb_customer
//            $objQuery =& SC_Query_Ex::getSingletonInstance();
//            $arrResult = $objQuery->select('customer_id,name01,name02', 'dtb_customer');
        }
